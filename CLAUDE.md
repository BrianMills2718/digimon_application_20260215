...
1. **Agent benchmark** (`eval/run_agent_benchmark.py`) — agent freely composes operators via MCP
   ...
   - Options: `--model codex` `--effort high` `--timeout 0` (default; disables hard per-question timeout) `--turn-timeout 0` (default; disables per-call timeout) `--stagnation-action observe|force_final` `--[no-]suppress-control-loop-calls` `--mode adaptive` `--resume` `--backend direct`
