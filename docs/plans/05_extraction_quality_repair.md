...
- 2026-03-22: That representation policy is now documented in the actual prompt surfaces too. The graph-building extraction prompts tell the build path to promote standalone entities only when they are likely to be reusable retrieval anchors, bridges, or relationship endpoints, while the benchmark retrieval prompts now explicitly remind the agent that the graph is a retrieval-optimized view rather than an exhaustive ontology and that missing standalone nodes may require pivoting to edge/chunk evidence.
- 2026-03-22: The next representation decision is now benchmark-grounded rather than ontology-first. [Plan #12](12_musique_representation_audit.md) freezes a MuSiQue reasoning-pattern audit before further extraction-policy changes are promoted, because the current benchmark evidence suggests the next high-value gaps are anchor disambiguation and composition semantics, not automatic ontology expansion.
